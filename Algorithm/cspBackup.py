...
class cspBackup:
    ...
    def __init__(self, degree:str, status:str, track:str, min_credit:int, max_credit:int, course_taken_unit:list, course_list:list, fullOrPartTime:str, currentSemester:str, currentYear:str,  insructionalStyle: str, program:str, courseId:str, foreignLanguage:str, prerequisites:list, required_program_courses:list, username:str, electives:list):
        self.degree = degree
        self.status = status
        self.track = track
        ...
        self.insructionalStyle = insructionalStyle
            ...
            for prerequisite in self.prerequisites:
                if prerequisite.get("courseId") == course.get("courseId"):
                    ...
                    if self.status == "Junior": 
                        if "Senior" in prerequisite.get("prerequisiteId") or "sophmore" in prerequisite.get("prerequisiteId") or "Sophmore" in prerequisite.get("prerequisiteId") or "senior" in prerequisite.get("prerequisiteId") or "first year" in prerequisite.get("prerequisiteId") or "first-year" in prerequisite.get("prerequisiteId") or "second year" in prerequisite.get("prerequisiteId") or "second-year" in prerequisite.get("prerequisiteId"):
                            removeCourse = True
                            print("REMOVE Status")
                            print(course.get("courseName"))
                            break
            ...
            for prerequisite in prerequisites:
                ...
                if prerequisite.get("courseId") == course.get("courseId"):
                    ...
                    for newCourse in course_prereq_list:
                        ...
                        if newCourse.get("courseName") != "FYS-1104" and newCourse.get("courseName") != "CS-3454":
                            newCourseName = newCourse.get("courseName").replace("-"," ")
                        elif newCourse.get("courseName") == "FYS-1104":
                            newCourseName = "FYS-1104"
                        elif newCourse.get("courseName") == "CS-3454":
    ...
    def removeWrongSemester(self, course_list):
        ...
        # should not equal SUMMR or blank 
        ...
        if self.currentSemester == "Winter":
            ...
            for course in course_list:
                if course.get("semester") != "WNTER" and course.get("semester") != "MAY" and course.get("semester") != "WI&MA" and course.get("semester") != "SUMMR" and course.get("semester") != "":
                    newCourseList.append(course)

        elif self.currentSemester == "May":
            for course in course_list:
                if course.get("semester") == "MAY" or course.get("semester") == "WI&MA" or course.get("semester") == "ALL" and course.get("semester") != "SUMMR" and course.get("semester") != "":
                    newCourseList.append(course)
    ...
    def removeSpecialTopics(self, course_list):
        ...
        for course in course_list:
            if "Special Topics" not in course.get("courseTitle"):
                newCourseList.append(course)
        return newCourseList
    ...
    def removeInstructionalStyles(self, course_list, offCampus, internship, experiential, studioArt, practicum, independentStudy):
        ...
        for course in course_list:
            ...
            if "Off-Campus/Study Abroad" in course.get("instructionalStyle"):
                canBeAdded = False

            if "Internship" in course.get("instructionalStyle"):
                canBeAdded = False
            
            if "Experiential" in course.get("instructionalStyle"):
                canBeAdded = False

            if "Studio Art" in course.get("instructionalStyle"):
                canBeAdded = False

            if "Practicum" in course.get("instructionalStyle"):
                canBeAdded = False

            if "Independent Study" in course.get("instructionalStyle"):
                canBeAdded = False
    ...
    def remove3000Courses(self, course_list):
        ...
        if self.status == "Freshman":
            ...
            for course in course_list:
                print(course.get("courseName"))
                if  "-3" not in course.get("courseName"):
                    newCourseList.append(course)
            return newCourseList
    ...
    def remove4000Courses(self, course_list):
        ...
        if self.status == "Freshman" or self.status == "Sophomore":

            for course in course_list:
                print(course.get("courseName"))
                if  "-4" not in course.get("courseName"):
                    newCourseList.append(course)
            return newCourseList
                    ...
                    if self.track in required_program_course.get("trackName") or self.track == "null":
                        if self.status in required_program_course.get("status"):
                            if self.currentSemester in required_program_course.get("semester"):
                                ...
                                #print(required_program_course.get("courseId"))
                                #print(course.get("courseId"))
                                if str(required_program_course.get("courseId")) == taken_course:
                                    required_courses_in_schedule = required_courses_in_schedule + 1
                                #else:
                                    #degreeGoal = False
    ...
    def backtracking_hard_constraint(self, possible_variables, assignment, credit):
        ...
        if credit >= int(self.min_credit):
            thershold = 1
            for course in assignment:
               if self.insructionalStyle in course.get("instructionalStyle"):
                   schedule_score = schedule_score + (1 / len(assignment))
               if self.program in course.get("program"):
                   schedule_score = schedule_score + (1 / len(assignment))
               if str(self.courseId) in str(course.get("courseId")):
                   schedule_score = schedule_score + (2 / len(assignment))
...
#        # a potiential schedule will be added to the set_solutions if it meets the credit requirement goals
    ...
    def hard_constraint_solutions(self, filename):
        ...
        if filename == self.username + "Profile1" and set_solutions:
            ...
            if csp.progress_check(self, self.username + "NewProfile", set_solutions[optimalIndex][0]) == False:
                ...
                print("boom")
                set_solutions.remove(set_solutions[optimalIndex])
                csp.hard_constraint_solutions(self, username + "Profile1")
                ...
                solution_index = 0
                while solution_index <  len(set_solutions[optimalIndex][0]):
                    schedule.write(str(set_solutions[optimalIndex][0][solution_index][1]) + " " + str(set_solutions[optimalIndex][0][solution_index][2]) + " " + str(set_solutions[optimalIndex][0][solution_index][3]) + " " + "\n")
                    solution_index = solution_index + 1
                schedule.write("\n")
        ...
        # and also send it to newProfile
            ...
            if csp.progress_check(self, "temp", set_solutions[optimalIndex][0]) == True:
                ...
                # Replace newProfile with temp file
                os.replace("temp.txt", self.username + "NewProfile.txt")
    ...
    def progress_check(self, filename, solutions):
        ...
        for line in file:
            ...
            if found_course_taken_unit:
                ...
                for course in courseList:
                    ...
                    if str(course.get("courseId")) in line:
                        #print(course.get("courseId"))
                        #print("found")
                        find_purpose = True
                        foundCourse = course
                ...
                if find_purpose == True:
                    ...
                    for elective in self.electives:
                        if elective.get("programName") in self.degree:
                            if self.track in elective.get("trackName") or self.track == "null":
                                if str(elective.get("courseId")) in str(foundCourse.get("courseId")):
                                    electives = electives + 1
                                if self.degree == "BIO":
                                    if elective.get("electiveType") == "3000 Above":
                                       bioAboveElective = bioAboveElective + 1
                                    elif elective.get("electiveType") == "bio":
                                        bioElective = bioElective + 1
                                    elif elective.get("electiveType") == "scince":
                    ...
                    #math
                    if "1M" in course_purpose:
                        #math-1034
                        if str(foundCourse.get("courseId")) == "768":
                            math = foundCourse
                            course_used = True
                        #math-1144
        ...
        if self.status == "Junior":
            if self.currentSemester == "Fall":
                if progress_score < 11:
                    return False
            if self.currentSemester == "Winter":
                if progress_score < 13:
                    return False
            if self.currentSemester == "May":
        ...
        if self.status == "Senior":
            if self.currentSemester == "Fall":
                if progress_score < 16:
                    return False
            if self.currentSemester == "Winter":
                if progress_score < 17:
                    return False
            if self.currentSemester == "May":
