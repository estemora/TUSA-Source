...
function NotesPage() {
  ...
  const [searchRecipientTerm, setSearchRecipientTerm] = useState(''); // New state for recipient filter
  ...
  const fetchNotes = async (username) => {
    try {
      const receivedNotesUrl = `http://www.cs.transy.edu/TUSA/js-login/front/build/showAllNotes.cgi`;
  
      const response = await axios.get(receivedNotesUrl);
  ...
  const handleRecipientSearchChange = (e) => {
    ...
    console.log("Recipient search term:", e.target.value);
  };
  ...
  const handleRecipientFilterChange = (e) => { // New function to handle recipient filter change
    const { value } = e.target;
    setSearchRecipientTerm(value);
  };
  ...
  // Function to filter notes based on username and recipient
  const filterNotes = (notes, sender, recipient, senderSearchTerm, recipientSearchTerm) => {
    return notes.filter((note) => {
      const senderMatch = !senderSearchTerm || note.sender.toLowerCase().includes(senderSearchTerm.toLowerCase());
      const recipientMatch = !recipientSearchTerm || note.username.toLowerCase().includes(recipientSearchTerm.toLowerCase());
      return senderMatch && recipientMatch && (!sender || note.sender === sender) && (!recipient || note.recipient === recipient);
    });
  };
      ...
      <div className="below-nav">
        ...
        <main>
          <section className="title">
            <p className="title-text">{notesTitleVisible ? 'ALL ADVISEE NOTES' : 'MY NOTE'}</p>
          </section>
        </main>
        <div className="notes-section">
          ...
          {showNotes && (
            <div className="display-notes-section">
             {filterNotes(receivedNotes, null, null, searchSenderTerm, searchRecipientTerm).map((receivedNote, index) => {
                            ...
                            <div className="note">
                              ...
                              {showHideButtons && (
                                ...
                                <div className="note-body">
                                    <a>{truncateText(receivedNote.body, 200)}</a>
                                </div>
                                <div className="delete-button">
                                    {!showHideButtons && (
